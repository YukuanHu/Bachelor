...
\textbf{最优运输问题}: 
\begin{itemize}
\item 建立有效比较概率分布的几何工具 (\tnewroman GAN).
...
\begin{frame}{问题陈述}
	...
	\only<2>{\begin{equation}
			...
			\end{array}
		\end{equation}}
	\only<1>{其中$X,Y\in\mathbb{R}^{n\times n}$, $r=(r_1,\ldots,r_n)^T,\rho=(\rho_1,\ldots,\rho_n)^T\in\mathbb{R}_+^n$. 将$\{|r_i-r_j|\}$储存于$R=(r_{ij})\in\mathbb{R}^{n\times n}$, 其中
	$$r_{ij}=\left\{\begin{array}{ll}
		1/|r_i-r_j|, & i\ne j,\\
		0, & i=j.
	\end{array}\right.$$}
	\onslide<2>{问题\eqref{original problem element form}有两个运输工具, 它的目标:
	\begin{itemize}
	\item 极小化两运输工具$X,Y$的运输费用 (\eqref{original problem element form}目标函数的前两项);
	\item 当$r_j,r_k$较近, $X$应尽量不该将$i$往$j$运输, $Y$应尽量不该将$i$往$k$运输 (\eqref{original problem element form}目标函数的最后一项).
	\end{itemize}}
\end{frame}
...
$$\begin{array}{rl}
	...
	\st & \left(\one^T\otimes I\right)\vectorize(X)=\left(I\otimes \one^T\right)\vectorize(X)=\rho,\\
	 & \left(\one^T\otimes I\right)\vectorize(Y)=\left(I\otimes \one^T\right)\vectorize(Y)=\rho,\\
	 & \vectorize(I)^T\vectorize(X)=0,\vectorize(X)\ge0,\\
	 & \vectorize(I)^T\vectorize(Y)=0,\vectorize(Y)\ge0.
\end{array}$$
\end{enumerate}
...
\begin{frame}{\tnewroman{ADMM}算法简介 (续)}
	...
	\item 多块凸可分问题 (\sfred{He/Tao/Yuan '12}, \sfred{Cai/Han/Yuan '17}等): $n\ge3,\ell=0$, $\theta_1,\ldots,\theta_n$是凸函数. 可能发散 (\sfred{Chen, et al. '16}).$\Rightarrow$无假设的困境.\pause
...
\begin{frame}{数值实验--测试问题 (续)}
	...
	处的函数值为0.
	\end{itemize}}
	\only<2>{$n=5,10,15,20$, $(p,q)=(3,4)$, $\alpha=1,\beta=10^3,\epsilon=10^{-8}$.\\
	\begin{center}
...
\only<1>{	\begin{center}
		\small\textcolor[rgb]{0,0.25,0.604}{表 8: }使用现成算法
	\end{center}
	\tnewroman
\begin{table}[htbp]
	\centering
	...
	\begin{tabular}{c|c|c||c|c|c}
		...
		算法\ref{ADMM} & 0.0097 & \textbf{1.1722} & 算法\ref{ADMM} & 0.0375 & \textbf{4.4934}\\\hline
		SQP & \textbf{0.0033} & \textbf{1.1722} & SQP & \textbf{0.0039} & \textbf{4.4934}\\\hline
		\hline
		$n=5$ & 所耗时间 (s) & 目标值 & $n=20$ & 所耗时间 (s) & 目标值\\\hline
		算法\ref{ADMM} & 0.0438 & \textbf{2.7741} & 算法\ref{ADMM} & 28.2271 & \textbf{9.8692}\\\hline
		SQP & \textbf{0.0048} & \textbf{2.7741} & SQP & \textbf{0.6388} & 9.9096\\\hline
		\hline
		$n=30$ & 所耗时间 (s) & 目标值 & $n=40$ & 所耗时间 (s) & 目标值\\\hline
		算法\ref{ADMM} & 129.1006 & \textbf{24.9461} & 算法\ref{ADMM} & 177.2822 & \textbf{19.0920}\\\hline
...
\begin{table}[htbp]
	...
	\begin{tabular}{c|c|c||c|c|c}
		...
		算法\ref{ADMM} & 195.8105 & \textbf{42.8754} & 算法\ref{ADMM} & 507.7331 & \textbf{30.3166}\\\hline
		SQP & \textbf{132.2582} & 46.8143 & SQP & \textbf{472.3746} & 33.0913\\\hline
		\hline
		$n=70$ & 所耗时间 (s) & 目标值 & $n=80$ & 所耗时间 (s) & 目标值\\\hline
		算法\ref{ADMM} & \textbf{1440.9720} & 30.2839 & 算法\ref{ADMM} & \textbf{2275.5156} & \textbf{40.6460} \\\hline
