...
\end{equation}
这里$\theta_i:\mathbb{R}^{d_i}\mapsto(-\infty,+\infty]$, $\ell:\mathbb{R}^d\mapsto(-\infty,+\infty]$; $x_i\in\mathbb{R}^{d_i}$; $A_i\in\mathbb{R}^{m\times d_i}$, $b\in\mathbb{R}^m$. 问题\beq{general ADMM problem}的增广拉格朗日函数为
...
等式\beq{X KKT1},\beq{X KKT2}分别对应于问题\beq{original problem matrix form}\tnewroman{KKT}条件\beq{KKT}中的
\begin{subequations}
	\begin{equation}
...
\begin{frame}{停机准则与\tnewroman{KKT}违反度 (续)}
	...
	=&2R+Z^{k+1}R-\Phi^{k+1}-\lambda_1^{k+1}\one^T-\mu^{k+1}I+\boxed{\color{red}{(Z^{k+1}-Z^k)(\beta I-R)}},
	\end{aligned}$$
	\item 使用更新策略\beq{multiplier update relax}:
	$$\begin{aligned}
	2R&+Z^kR-\Phi^k+\beta(X^{k+1}-Z^k)-\lambda_1^{k+1}\one^T-\mu^{k+1}I\\
	=&2R+Z^{k+1}R-\Phi^{k+1}-\lambda_1^{k+1}\one^T-\mu^{k+1}I\\
	&+\boxed{{\color{red}{(Z^{k+1}-Z^k)(\beta I-R)}}}+\boxed{{\color{red}{(1-\alpha)\beta(X^{k+1}-Z^{k+1})}}}.
	\end{aligned}$$
	\end{itemize}
\end{frame}
...
\begin{subequations}
	...
	\end{equation}
\end{subequations}
\onslide<2>{等式\beq{Z KKT1}-\beq{Z KKT4}分别对应于问题\beq{original problem matrix form}\tnewroman KKT条件\beq{KKT}中的
\begin{subequations}
	\begin{equation}
...
\begin{frame}{停机准则与\tnewroman{KKT}违反度 (续)}
	...
	\item 使用更新策略\beq{multiplier update relax}:
		$$\begin{aligned}
		&X^{k+1}R-\Phi^k-\beta(X^{k+1}-Z^{k+1})-\one\left(\lambda_2^{k+1}\right)^T-\Omega^{k+1}\\
		=&X^{k+1}R-\Phi^{k+1}-\one\left(\lambda_2^{k+1}\right)^T-\Omega^{k+1}-\boxed{{\color{red}{(1-\alpha)\beta(X^{k+1}-Z^{k+1})}}}.
		\end{aligned}$$
	\end{itemize}
\end{frame}
...
\begin{itemize}
\item \tnewroman{MATLAB}内置函数\texttt{fmincon()}, 调用算法\texttt{'sqp','active-set'}求解问题\beq{original problem matrix form 2}和\beq{original problem matrix form 1}, 设置停机准则\texttt{ConstraintTolerance}为上文默认值. 在使用算法\ref{ADMM}求解$n=50,60,70,80$的问题, 设置惩罚因子
$$\left\{\begin{array}{ll}
	\beta=10^4, & n=50,60,\\
