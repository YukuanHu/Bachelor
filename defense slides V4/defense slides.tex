...
已有方法:
\begin{itemize}
\item 割平面法 (\sfred{Ritter '66}等)\tnewroman;
...
%\end{itemize}
\end{enumerate}
以上方法均以{\color{red}{列向量}}为求解对象$\Rightarrow$\textbf{\color{red}{计算量问题}}.
\end{frame}
...
\end{equation}
\begin{rem}
问题\blue{\eqref{original problem matrix form}}的{\color{red}{约束非常特殊}}, 但在最优运输问题中十分普遍.
\end{rem}
\end{frame}
...
\end{equation}
	现有工作简介:
	\begin{itemize}
	\item 凸可分问题 (\sfred{Boyd, et al. '10}, \sfred{He/Yuan '12}等): $n\ge2,\ell=0$, $\theta_1,\theta_2$是凸函数. 可能发散 (\sfred{Chen, et al. '16})$\Rightarrow$\textbf{无假设的困境};
	\item 凸不可分问题 (\sfred{Hong, et al. '14}, \sfred{Chen, et al. '19}), 非凸问题. 理论缺乏, 应用广泛. 
	\end{itemize}
	\textbf{特点}: {\color{red}{交替}}求解子问题, 子问题需要{\color{red}{易于求解}}; 适合于{\color{red}{分布式计算}}.\\
	$\Rightarrow$引入$Z$的理由之一.
\end{frame}
...
\begin{rem}[引入$Z$的理由之二]
若未{\color{red}{恰当}}引入分裂变量$Z$, $X$子问题的求解难度非常之大.
\end{rem}
\end{frame}
...
\section{数值实验}
\begin{frame}{数值实验}
使用\tnewroman{MATLAB}内置函数\texttt{randn()}和\texttt{abs()}生成随机问题. 小型问题以$n=3,4,5$各一问题为代表, 大型问题以$n=20,30,40$各一问题为代表. \\[1em]
\textbf{目的}:
\begin{itemize}
\item 揭示算法的{\color{red}{性质}}\tnewroman;
...
\begin{frame}{数值实验--松弛因子$\alpha$ (算法性质)}
...
\end{figure}
\begin{center}
	\small\textcolor[rgb]{0,0.25,0.604}{\tnewroman 图 1: }上$n=3$, 下$n=30$, 不同的松弛因子\\\footnotesize 左: 迭代数; 中: 所耗时间; 右: 目标值.
\end{center}
\end{frame}
...
\begin{frame}{数值实验--惩罚因子$\beta$ (算法性质)}
\only<1>{\begin{center}
	\small\textcolor[rgb]{0,0.25,0.604}{\tnewroman 表 1: }不同的惩罚因子
\end{center}
\tnewroman
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c|c|c|c|c}
        ...
        {\color{red}{100}} & \textbf{224} & \textbf{0.0043} & 5.46$\times10^{-9}$ & \textbf{1.1722}\\\hline
        ...
        {\color{red}{1000}} & \textbf{860} & \textbf{0.0384} & 9.54$\times10^{-9}$ & \textbf{2.7741}\\\hline
        ...
        \multirow{2}{*}{$\beta$} & \multicolumn{4}{c}{$n=30$}\\\cline{2-5}
          & 迭代数 & 所耗时间 (s) & KKT违反度 & 目标值\\\hline
        {\color{red}{10000}} & \textbf{207788} & \textbf{133.6154} & \textbf{6.00$\mathbf{\times10^{-7}}$} & \textbf{24.9461}\\\hline
...
\end{figure}
\begin{center}
	\small\textcolor[rgb]{0,0.25,0.604}{\tnewroman 图 2: }$n=30,\beta=10^3$残差陷入循环\\\footnotesize 左: 原始残差; 右: 对偶残差.
\end{center}
}
\end{frame}
...
\begin{center}
	...
	\end{center}
\textbf{结论}: 对于大型问题, 算法\ref{ADMM}更具优势.
\end{frame}
